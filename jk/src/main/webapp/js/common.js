...
function CheckAtts() {
    ...
    for (i = 0; i < DarkTR.length; i++) {
        if (attentionArray.indexOf(DarkTR[i].parentNode.parentNode.parentNode.id) != -1) {
            ...
            top.middle.switches.loading2.style.display = 'none';
            /*加载完毕后，关闭LOADING图示*/
            DarkTR[i].innerHTML = TEXT + '<img style=\"margin-left:10px;\" src=\"../skin/default/images/notice.gif\">';
            /*自动为已加入关注的内容后增加对应文字或图示*/
            //alert('YES');
        }
    }
...
function ShowChangeTD(obj) {
    ...
    if (obj.className == "CTM_row_dark_focus") {
        top.middle.switches.loading2.style.display = 'block';
        /*定义点击未增加关注的TD后，弹出LOADING图示*/
